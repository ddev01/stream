...
public class CPHInline
{
    ...
    private List<object> ReadSubscriptionHistoryFromDb()
    {
        try
        {
            ...
            foreach (var doc in results)
            {
                ...
                if (string.IsNullOrEmpty(oid) || string.IsNullOrEmpty(subscribedAtDate) || 
                    string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(gifterUserId))
                {
                ...
                {
                    ["_id"] = new Dictionary<string, object> { ["$oid"] = oid },
                    ["subscribedAt"] = new Dictionary<string, object> { ["$date"] = subscribedAtDate },
                    ["userId"] = userId,
    ...
    private bool PostToApi(List<object> subscriptionHistory)
    {
        ...
        {
            // Prepare payload
            var payload = new { subscription_history = subscriptionHistory };
