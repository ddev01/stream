...
<x-layouts.app :title="__('Home')">
	<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
		<div class="grid auto-rows-min gap-4 md:grid-cols-3">
			...
			<x-leaderboard-card
				title="Top 5 Watchtime"
				:top-stats="$topWatchtime"
				:user-stat="$userWatchtimeStats"
				:user-position="$userWatchtimePosition"
				:value-formatter="fn($value) => TimeFormatter::formatWatchtime($value)"
			/>

			<!-- Top Three Count Leaderboard -->
			<x-leaderboard-card
				title="Top 5 Top Three"
				:top-stats="$topTopThree"
				:user-stat="$userTopThreeStats"
				:user-position="$userTopThreePosition"
			/>
		</div>
