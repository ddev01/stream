...
<th {{
    $attributes->merge($customThAttributes)
        ->class([
            'text-gray-500 dark:bg-neutral-900 dark:text-neutral-400' => $isTailwind && (($customThAttributes['default-colors'] ?? true) || ($customThAttributes['default'] ?? true)),
            ...
            @if ($isTailwind)
                ...
                <button wire:click="sortBy('{{ $column->getColumnSortKey() }}')" {{
                        $attributes->merge($customSortButtonAttributes)
                            ->class([
                                'text-gray-500 dark:text-neutral-400' => (($customSortButtonAttributes['default-colors'] ?? true) || ($customSortButtonAttributes['default'] ?? true)),
